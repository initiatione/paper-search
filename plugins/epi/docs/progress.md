...
- 论文发现 skill bundle：`paper-discovery` 已蒸馏 `nature-academic-search` 的结构化组织方式，新增中文 README、离线 query planner、domain ontology、source tier、dedup engine、venue prior、two-stage retrieval、citation graph、evaluation set、multi-source workflow 等分层参考文件；RoboWiki 这类机器人会议/期刊整理页可作为 curated venue prior，知乎/论坛榜单只能作为弱召回线索，影响因子、分区、引用数等仍需实时或可信来源核验。
- 精准检索计划：`skills/paper-discovery/scripts/query-planner.py` 可把用户主题转换成 `query_plan`，包含领域、概念块、5-8 条 query variants、source route、recall gap checks 和 quality signals；AUV/control 请求会自动展开 AUV/UUV、RL/offline RL/model-based RL、tracking/path following/stabilization、ocean current/current disturbance、sea trial/real AUV/sim-to-real 等术语。默认 `dry-run` 已接入 query plan，会写 `query-plan.json`，按 query variants 多次搜索并合并 candidate pool；默认排除 review/survey/meta 类综述候选，用户明确找综述时才放开；`--no-query-plan` 仅用于单 query 调试。
