...
更新时间：2026-06-06。本文档记录当前源码树的开发状态、已完成能力、已知风险和下一步计划。它是面向继续开发和发布验收的进度页，不替代 `docs/epi-linkage.md` 的链路契约，也不替代 `docs/structure.md` 的结构说明。
...
- Workflow boundary source version sync：本轮 EPI/PRW 工作流边界同步先把 source manifest 从 EPI `0.1.12` / PRW `0.1.5` bump 到 EPI `0.1.13` / PRW `0.1.6`；随后 ask-mode record automation 再 bump 到 EPI `0.1.14` / PRW `0.1.7`。installed cache 仍需 marketplace refresh / reinstall 后才会从 EPI `0.1.11`、PRW `0.1.4` 更新。
