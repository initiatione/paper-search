...
- Source-staging 和报告：默认 fast-ingest 在源材料完整后生成 `wiki-ingest-brief.json`、`promotion-plan.json` 和 `briefs/reading-report.md`；audited-ingest 需要 critic pass。阅读报告是中文优先的人工确认报告，不是正式 wiki 页面。
