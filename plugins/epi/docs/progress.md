...
本轮 paper-search MCP 深度使用优化范围是：把 upstream MCP 从“浅层搜索/下载包装”提升为 EPI 的多源发现、开放访问采集、provider readiness 诊断和检索预览能力边界；EPI 继续负责 query plan、过滤、排序、source bundle、human gate 和 PRW handoff。插件版本同步到 `0.1.7`，源码验证通过后仍需单独刷新 installed cache 才能成为 live runtime。`0.1.7` 在 `0.1.6` 的 retrieval preview 基础上新增 `paper_search_provider_readiness` 和 source capability matrix：`doctor --json` 会暴露 Unpaywall email、CORE key、Semantic Scholar key、Google Scholar proxy、DOAJ key、Zenodo token 等 provider gap；`Source Coverage` 会展示 `capabilities` 和 `provider_readiness`，让 source 的 download/read 支持状态与 OA fallback 环境缺口进入报告，而不是停留在隐藏配置。
...
结果：MCP/read-preview focused 回归为 `63 passed in 14.16s`；PRW/EPI contract 回归为 `40 passed in 0.47s`；审查后补测 source coverage dedupe、info-only read preview 过滤和 fallback success probe 一致性为 `3 passed in 2.96s`；全量 `tests\epi` 为 `391 passed in 53.81s`；`plugins\epi\tests` 为 `12 passed in 0.44s`；EPI plugin validation passed；PRW plugin validation passed；`git diff --check` 无 whitespace error，仅有 Windows LF/CRLF 提示。当前 `plugin.json` 版本为 `0.1.7`，installed cache 仍需单独 refresh/reinstall 才代表 live runtime。
