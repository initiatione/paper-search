...
def main() -> int:
    ...
    ingest_one.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    acquire = subparsers.add_parser("acquire-paper")
    acquire.add_argument("--candidate", type=Path, required=True)
    acquire.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    advance = subparsers.add_parser("advance-paper")
    advance.add_argument("--candidate", type=Path, required=True)
    advance.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    ...
    advance_batch.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    ...
    advance_ranked.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    ...
    parse_paper.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    ...
    promote.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    ...
    rollback.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    rollback.add_argument("--slug", required=True)
    redo_acquire_parser = subparsers.add_parser("redo-acquire")
    redo_acquire_parser.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    redo_acquire_parser.add_argument("--slug", required=True)
    redo_acquire_parser.add_argument("--pdf", type=Path, required=True)
    redo_acquire_parser.add_argument("--reason", default="")
    redo_parse_parser = subparsers.add_parser("redo-parse")
    redo_parse_parser.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    redo_parse_parser.add_argument("--slug", required=True)
    redo_parse_parser.add_argument("--mineru-md", type=Path, required=True)
    redo_parse_parser.add_argument("--mineru-tex", type=Path, default=None)
    redo_parse_parser.add_argument("--mineru-images", type=Path, default=None)
    redo_parse_parser.add_argument("--reason", default="")
    redo_read_parser = subparsers.add_parser("redo-read")
    redo_read_parser.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    redo_read_parser.add_argument("--slug", required=True)
    redo_read_parser.add_argument("--reason", default="")
    recritic_parser = subparsers.add_parser("recritic")
    recritic_parser.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    recritic_parser.add_argument("--slug", required=True)
    recritic_parser.add_argument("--reason", default="")
    ...
    feedback.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    ...
    propose.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    ...
    activate.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    ...
    runs_query.add_argument("--vault", type=Path, default=Path(r"D:\paper-research-wiki"))
    ...
    if args.command == "ingest-one":
        ...
        return 0
    if args.command == "acquire-paper":
        candidate = json.loads(args.candidate.read_text(encoding="utf-8"))
        record = acquire_paper_from_candidate(args.vault, candidate)
        print(f"acquire_status={record['status']}")
        print(f"paper_root={raw_paper_root(args.vault, candidate['slug'])}")
        return 0 if record["status"] == "success" else 1
    if args.command == "advance-paper":
        candidate = json.loads(args.candidate.read_text(encoding="utf-8"))
        state = advance_paper_once(args.vault, candidate, mineru_command=args.mineru_command)
    ...
    if args.command == "advance-batch":
        candidates = json.loads(args.candidates.read_text(encoding="utf-8"))
        batch = advance_paper_batch(
            args.vault,
            candidates,
    ...
    if args.command == "advance-ranked":
        batch = advance_paper_batch_from_run(
            args.vault,
            args.run_id,
            mineru_command=args.mineru_command,
            max_papers=args.max_papers,
        ...
        print(f"processed_count={batch['processed_count']}")
        return 0 if batch["state"] != "batch_failed" else 1
    if args.command == "parse-paper":
        record = parse_paper_with_mineru(args.vault, args.slug, mineru_command=args.mineru_command)
        print(f"parse_status={record['status']}")
        if record.get("batch_id"):
            print(f"batch_id={record['batch_id']}")
        return 0 if record["status"] == "success" else 1
    if args.command == "promote-to-wiki":
        vault_path = args.vault.resolve()
        run_id, run_dir = _new_run_dir(vault_path, "promote-to-wiki")
        started_at = utc_now()
        record = promote_paper(args.vault, args.slug, approved_by=args.approved_by)
        ...
        _write_promotion_or_rollback_run_state(
            ...
            input_artifact_hashes={
                "promotion-record.json": file_sha256(promotion_record_path),
            },
        ...
        return 0
    if args.command == "rollback-promotion":
        vault_path = args.vault.resolve()
        run_id, run_dir = _new_run_dir(vault_path, "rollback-promotion")
        started_at = utc_now()
        record = rollback_promotion(args.vault, args.slug)
        paper_root = raw_paper_root(vault_path, args.slug)
        promotion_record_path = paper_root / "promotion-record.json"
        rollback_record_path = paper_root / "rollback-record.json"
        promotion_record = json.loads(promotion_record_path.read_text(encoding="utf-8"))
        ...
        return 0
    if args.command == "redo-acquire":
        started_at = utc_now()
        record = redo_acquire(args.vault, args.slug, args.pdf, reason=args.reason)
        _write_repair_routed_report(
            args.vault.resolve(),
            args.slug,
            record,
            started_at=started_at,
    ...
    if args.command == "redo-parse":
        ...
        print(f"redo_status={record['status']}")
        return 0
    if args.command == "redo-read":
        started_at = utc_now()
        record = redo_read(args.vault, args.slug, reason=args.reason)
        _write_repair_routed_report(
            args.vault.resolve(),
            args.slug,
            record,
        ...
        print(f"redo_status={record['status']}")
        return 0
    if args.command == "recritic":
        started_at = utc_now()
        record = recritic(args.vault, args.slug, reason=args.reason)
        _write_repair_routed_report(
            args.vault.resolve(),
            args.slug,
            record,
    ...
    if args.command == "record-feedback":
        feedback_record = record_feedback(
            args.vault,
        ...
        return 0
    if args.command == "propose-evolution":
        ...
        return 0
    if args.command == "activate-evolution":
        activated = activate_evolution(args.vault, args.proposal_id, approved=args.approved)
        print(f"evolution_status={activated['status']}")
        return 0
    if args.command == "runs-query":
        result = query_runs(
            args.vault.resolve(),
            failed=args.failed,
        ...
        print(render_runs_query(result))
        return 0
    return 2
