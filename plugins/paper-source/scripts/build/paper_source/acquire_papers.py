...
from paper_source.artifacts import file_sha256, json_sha256, quarantine_root, utc_now, write_json_atomic
from paper_source.paper_search_adapter import download_paper_pdf, read_paper_preview
...
def _manual_download_context(candidate: dict) -> dict | None:
    ...
    return {
        ...
        "preferred_next_step": (
            ...
            "then provide the local PDF or a direct open-access PDF URL to Paper Source."
        ),
        "tmp_manual_pdf_dir": "_paper_source/tmp-manual-pdfs",
...
def _classify_acquire_failure(http_status: int | None, error_kind: str | None = None) -> tuple[str, bool, str]:
    ...
    string. The hint is advisory only; Paper Source never auto-retries or auto-switches
    sources on its own.
    """
...
def _download_url_to_temp(url: str, temp_pdf: Path, timeout_seconds: int) -> tuple[int | None, str | None]:
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 (compatible; PaperSource/0.1; +https://github.com/initiatione/paperflow)",
