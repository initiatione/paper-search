...
from paper_source.artifacts import raw_paper_root, staging_paper_root, utc_now, write_json_atomic, write_text_atomic
from paper_source.run_critic import HARD_RULE
from paper_source.wiki_contracts import formal_page_family_names, formal_page_family_paths
...
def _research_decision_frontmatter_lines(summary: dict) -> list[str]:
    lines = [
        f"paper_source_recommendation: {_frontmatter_value(summary.get('recommendation', ''))}",
        f"paper_source_next_action: {_frontmatter_value(summary.get('next_action', ''))}",
        f"paper_source_panel_consensus: {_frontmatter_value(summary.get('panel_consensus', ''))}",
        f"paper_source_blocking_lenses: {_frontmatter_value(summary.get('blocking_lenses') or [])}",
        f"paper_source_warning_reviewers: {_frontmatter_value(summary.get('warning_reviewers') or [])}",
    ]
...
def _ensure_reference_frontmatter(path: Path, summary: dict | None) -> None:
    ...
    if "paper_source_panel_consensus:" in text or "epi_panel_consensus:" in text:
        return
...
def _refresh_index_and_hot(vault_path: Path, manifest: dict) -> None:
    ...
    write_text_atomic(
        ...
        _replace_managed_section(
            index_existing,
            "<!-- PAPER_SOURCE:PROMOTED-PAPERS:START -->",
            "<!-- PAPER_SOURCE:PROMOTED-PAPERS:END -->",
    ...
    write_text_atomic(
        ...
        _replace_managed_section(
            hot_existing,
            "<!-- PAPER_SOURCE:HOT-PAPERS:START -->",
            "<!-- PAPER_SOURCE:HOT-PAPERS:END -->",
...
def promote_paper(vault_path: Path, slug: str, approved_by: str | None = None) -> dict:
    ...
    raise ValueError(
        "promote-to-wiki is deprecated: Paper Source may only write internal underscore artifacts. "
