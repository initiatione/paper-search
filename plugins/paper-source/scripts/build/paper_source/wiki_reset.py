                        ...
                        if preserve_config and meta_child.name in PROTECTED_META_NAMES:
                            ...
                            actions.append(
                                _action_record(
                                    "would_backup",
                                    meta_child,
                                    effective_backup_root / child.name / "meta" / meta_child.name,
                                )
                            )
                for internal_child in sorted(child.iterdir(), key=lambda item: item.name):
                    if internal_child.name == "meta":
                        continue
                    if effective_backup_root is None:
                        actions.append(_action_record("would_remove", internal_child))
                    else:
                        actions.append(_action_record("would_backup", internal_child, effective_backup_root / child.name / internal_child.name))
                continue
            if child.name == "_meta":
...
def reset_wiki_vault(
    ...
    *,
                ...
                if meta_dir.exists():
                    actions.extend(
                        _reset_meta_dir(
                            ...
                            backup_root,
                            preserve_config=preserve_config,
                            backup_relative_root=Path(child.name) / "meta",
                        )
                    )
                for internal_child in sorted(child.iterdir(), key=lambda item: item.name):
                    if internal_child.name == "meta":
                        continue
                    if backup_root is None:
                        _remove_path(internal_child)
                        actions.append(_action_record("remove", internal_child))
                    else:
                        moved_to = _move_path(internal_child, backup_root / child.name)
                        actions.append(_action_record("backup", internal_child, moved_to))
                continue
    ...
    manifest_dir = paper_source_meta_root(vault_path) / "wiki-reset"
