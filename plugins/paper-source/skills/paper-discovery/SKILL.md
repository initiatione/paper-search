...
description: >
  Use when the user asks Paper Source / PS to find academic papers with Paper Source, including "找最新论文",
...
`dry-run` writes only `_paper_source/runs/<run-id>/`. `prepare-ranked` belongs to `paper-ingest` and stops at source-staging. Final Obsidian/LLM Wiki pages are written by Paper Wiki `$paper-research-wiki` or, for compatibility, through `paper-source-paper-deposition`; discovery never writes them.
