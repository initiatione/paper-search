...
This writes `_paper_source/staging/papers/<slug>/human-approval.json` and changes the handoff to `ready_for_agent=true`.
...
`wiki-ingest-trigger` writes `_paper_source/staging/papers/<slug>/wiki-agent-trigger.json`, a machine-readable resume package for Paper Wiki `$paper-research-wiki` or another wiki-capable agent following the Paper Wiki/vault contract. It does not spawn a hidden background LLM process and does not write final pages. The trigger points at `wiki-ingest-brief.json`, the canonical Paper Source-to-Paper Wiki handoff; `wiki_deposition_task.json is legacy` compatibility only.

## Record Final Wiki Ingest

After Paper Wiki `$paper-research-wiki` or another wiki-capable agent following the Paper Wiki/vault contract writes or stages final Markdown pages, create `final-source-review.json`, then record completion:
...
For Paper Wiki ask-mode automation, Paper Wiki writes the request artifact; Paper Source consumes it:
...
If a prior `wiki-ingest-record.json` was corrected as `premature-wiki-ingest-record`, the correction entry is under `_paper_source/meta/record-corrections/`. While its wiki status is `pending-paper-wiki-review`, send the paper to Paper Wiki for formal page and `final-source-review.json` repair. After Paper Wiki repairs pages and `final-source-review.json`; Paper Source writes or replaces `wiki-ingest-record.json`: when the correction status is `paper-wiki-reviewed-ready-for-paper-source-record`, `paper-gate` returns `ready_for_wiki_ingest_agent`; rerun `record-wiki-ingest` to replace the premature record. Legacy `pending-prw-review` and `prw-reviewed-ready-for-epi-record` remain accepted only for existing correction artifacts. Paper Wiki reports readiness but must not write the Paper Source replacement record.
