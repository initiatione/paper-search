...
When enabled by the acquisition path, upstream `read_<source>_paper` or CLI read may create `paper-search-read-preview.txt` as a non-authoritative preview sidecar and record `retrieval_preview`. This preview is subject to the same upstream paper indexes and provider terms, and it is not replacing MinerU or final source-first wiki review.
