...
Naming: Paper Wiki is this plugin's user-visible name. PRW remains a legacy alias and the current machine-facing plugin name remains `prw`. Paper Source is the source-preparation sibling plugin; EPI remains a legacy alias and the current machine-facing plugin name remains `epi`. PW/PS are conversational aliases only.
