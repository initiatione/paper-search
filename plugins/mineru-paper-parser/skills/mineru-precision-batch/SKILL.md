---
name: mineru-precision-batch
description: "Use this skill when the user wants to parse local PDFs or document batches into organized Markdown with MinerU's precise batch API, especially engineering or research papers that need formula, table, and image extraction. Defaults to the token-authenticated precise batch API, relative project paths, VLM model, English language, and one folder per paper such as output/paper/paper.md plus output/paper/images/."
---
...
```powershell
python skills/mineru-precision-batch/scripts/mineru_batch_to_md.py --input-dir paper --output-dir parsed
```

For a sibling workspace, still use a path relative to the project root:

```powershell
python skills/mineru-precision-batch/scripts/mineru_batch_to_md.py --input-dir ..\human_robot\paper --output-dir parsed
```

If a batch has already completed and only the local outputs need to be rebuilt, reuse the batch id without re-uploading:

```powershell
python skills/mineru-precision-batch/scripts/mineru_batch_to_md.py --batch-id <batch_id> --input-dir ..\human_robot\paper --output-dir parsed
```
