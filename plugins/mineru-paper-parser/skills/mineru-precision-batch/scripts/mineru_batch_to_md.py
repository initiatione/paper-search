...
def main() -> int:
    parser = argparse.ArgumentParser(description="Parse local PDFs with MinerU precise batch API into per-paper Markdown and image folders.")
    ...
    parser.add_argument("--batch-id", help="Reuse an existing MinerU batch id and download its outputs without re-uploading files.")
    parser.add_argument("--no-extract-images", action="store_true", help="Save Markdown only. By default, images/ assets are extracted too.")
    ...
    if not token:
        raise RuntimeError(f"MINERU_TOKEN is required. Set it in the environment or {relative_text(project_root, env_file)}")
