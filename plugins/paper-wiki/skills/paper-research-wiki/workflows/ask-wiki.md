...
This workflow is read-only. Use no `log.md` writes: do not write `log.md`, do not write formal pages, do not refresh QMD, and do not write Paper Source artifacts.

This `ask_wiki` workflow is the conversational primary entrypoint. Paper Source `wiki-ask` CLI is the same-source fallback / 程序化 `--json` entry against the same formal graph.
...
4. Use `_paper_source/raw` as primary source evidence when formal pages need confirmation, and use legacy `_epi/raw` only for existing artifacts. Do not treat `_paper_source/**` or legacy `_epi/**` as formal graph pages.
...
7. Track correction candidates during retrieval. Include broken wikilinks, ambiguous aliases, duplicate concept owners, stale tracking links, forbidden `_paper_source/**` or legacy `_epi/**` graph links, source/frontmatter mismatch, and relationship direction mistakes.
8. Stop after the answer. Report correction candidates and ask before repair. Do not route to update, relink, redo, `record-wiki-ingest`, `paper-wiki-record-request.json`, or any write workflow until the user confirms.
