...
Paper Wiki maintains formal pages only. Do not rename files under `_paper_source/raw/**`. If `figure-index.json`, `formula-index.json`, or `asset-normalization-record.json` is missing, report that Paper Source raw bundle normalization must run first.
...
3. Find raw MinerU image paths (`../_paper_source/raw/.../mineru/images/<hash>...`), Markdown embeds into raw image directories, `image images/<hash>` source lines, and evidence lines mentioning `Fig.`, `Figure`, `图`, `Table`, `Eq.`, or formula image evidence.
...
1. Create a pre-repair snapshot under `_paper_source/meta/formal-page-snapshots/<timestamp>-pre-figure-repair/` with a hash manifest for every changed formal page.
