...
Ar9av/obsidian-wiki is a design source, not a runtime source of truth. During normal Paper Wiki runs, do not fetch or search Ar9av/obsidian-wiki. Use the local Paper Wiki workflows, references, rules, and the target vault contract as the executable contract. Consult the upstream repository only when maintaining Paper Wiki, auditing drift, or deliberately upgrading the adapted rules.
...
Every write-oriented workflow ends with a post-task check: rerun the Paper Wiki check workflow, summarize remaining link/provenance/QMD/index issues, and only then tell the user whether Paper Source `record-wiki-ingest` remains.
