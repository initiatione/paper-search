...
def _answers(**overrides):
    payload = {
        ...
        "zotero_collection": "Paper Source",
...
def test_init_config_writes_wiki_config_state_and_history(tmp_path, monkeypatch, capsys):
    ...
    history_files = list((vault / "_paper_source/meta" / "config-history").glob("*.yaml"))
...
def test_propose_config_update_does_not_write_until_apply(tmp_path, monkeypatch, capsys):
    ...
    _run_orchestrator_cli(monkeypatch, capsys, "init-config", "--vault", str(vault), "--answers-json", str(answers_path))
    before = (vault / "_paper_source/meta" / "paper-source-config.yaml").read_text(encoding="utf-8")
    ...
    after_propose = (vault / "_paper_source/meta" / "paper-source-config.yaml").read_text(encoding="utf-8")
    ...
    config_text = (vault / "_paper_source/meta" / "paper-source-config.yaml").read_text(encoding="utf-8")
    history_files = list((vault / "_paper_source/meta" / "config-history").glob("*.yaml"))
...
def test_reset_update_preserves_existing_wiki_artifacts(tmp_path, monkeypatch, capsys):
    vault = tmp_path / "vault"
    for relative in [
        "_paper_source/raw/keep/paper.pdf",
        "_paper_source/runs/keep-run/run-state.json",
        "_paper_source/staging/papers/keep/promotion-plan.json",
        "references/keep.md",
    ]:
        path = vault / relative
        path.parent.mkdir(parents=True, exist_ok=True)
    ...
    assert exit_code == 0
    assert "profile: reset_profile" in (vault / "_paper_source/meta" / "paper-source-config.yaml").read_text(encoding="utf-8")
    assert (vault / "_paper_source/raw/keep/paper.pdf").read_text(encoding="utf-8") == "keep"
    assert (vault / "_paper_source/runs/keep-run/run-state.json").read_text(encoding="utf-8") == "keep"
    assert (vault / "_paper_source/staging/papers/keep/promotion-plan.json").read_text(encoding="utf-8") == "keep"
...
def test_wiki_reset_preserves_config_by_default_and_backs_up_content(tmp_path, monkeypatch, capsys):
    ...
    _run_orchestrator_cli(monkeypatch, capsys, "init-config", "--vault", str(vault), "--answers-json", str(answers_path))
    raw_paper = vault / "_paper_source/raw" / "paper-a" / "paper.pdf"
    raw_paper.parent.mkdir(parents=True)
    raw_paper.write_text("paper", encoding="utf-8")
    transient_meta = vault / "_paper_source/meta" / "temporary.json"
    ...
    assert "profile: kept_profile" in (vault / "_paper_source/meta" / "paper-source-config.yaml").read_text(encoding="utf-8")
    assert not raw_paper.exists()
    assert (backup_root / "_paper_source/raw" / "paper-a" / "paper.pdf").read_text(encoding="utf-8") == "paper"
    assert (backup_root / "_paper_source/meta" / "temporary.json").read_text(encoding="utf-8") == "temporary"
    assert (vault / "_paper_source/meta" / "wiki-reset").is_dir()
...
def test_wiki_reset_preview_lists_actions_without_mutating(tmp_path, monkeypatch, capsys):
    ...
    assert "profile: preview_profile" in (vault / "_paper_source/meta" / "paper-source-config.yaml").read_text(encoding="utf-8")
...
def test_wiki_reset_requires_separate_config_confirmation(tmp_path, monkeypatch, capsys):
    ...
    assert not (vault / "_paper_source/meta" / "paper-source-config.yaml").exists()
    assert (vault / "_paper_source/raw").is_dir()
...
def test_config_recover_lists_backup_candidates_and_restore_requires_confirmation(tmp_path, monkeypatch, capsys):
    ...
    assert "profile: recovered_profile" in (vault / "_paper_source/meta" / "paper-source-config.yaml").read_text(encoding="utf-8")
    state = json.loads((vault / "_paper_source/meta" / "paper-source-config-state.json").read_text(encoding="utf-8"))
...
def test_config_recover_prioritizes_current_and_history_before_backups(tmp_path, monkeypatch, capsys):
    vault = tmp_path / "vault"
    current = vault / "_paper_source/meta" / "paper-source-config.yaml"
    history = vault / "_paper_source/meta" / "config-history" / "old.yaml"
    backup = tmp_path / "reset-backups" / "old-run" / "_paper_source/meta" / "paper-source-config.yaml"
...
def test_wiki_repair_can_restore_missing_config_from_backup(tmp_path, monkeypatch, capsys):
    ...
    assert "profile: repair_profile" in (vault / "_paper_source/meta" / "paper-source-config.yaml").read_text(encoding="utf-8")
