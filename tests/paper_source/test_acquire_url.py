...
from paper_source.acquire_papers import acquire_paper_from_url, _classify_acquire_failure
from paper_source.orchestrator import acquire_paper_from_candidate
...
def test_acquire_paper_from_url_downloads_pdf_and_records_metadata(tmp_path):
    ...
    paper_root = tmp_path / "vault" / "_paper_source" / "raw" / "downloaded-paper"
...
def test_acquire_paper_from_url_records_http_failure_without_pdf(tmp_path):
    server_root = tmp_path / "server"
    server_root.mkdir()
    paper_root = tmp_path / "vault" / "_paper_source" / "raw" / "missing-paper"
...
def test_acquire_paper_from_url_quarantines_identity_mismatch_before_success(tmp_path):
    ...
    paper_root = vault / "_paper_source" / "raw" / "identity-mismatch-paper"
    ...
    quarantine_pdf = vault / "_paper_source" / "quarantine" / "papers" / "identity-mismatch-paper" / "paper.pdf"
...
def test_acquire_paper_from_url_follows_landing_page_citation_pdf_url(tmp_path):
    ...
    paper_root = tmp_path / "vault" / "_paper_source" / "raw" / "landing-paper"
...
def test_acquire_paper_from_url_tries_candidate_pdf_url_fallbacks(tmp_path):
    ...
    server_root.mkdir()
    (server_root / "paper.pdf").write_bytes(b"%PDF-1.4\nfallback fixture\n")
    paper_root = tmp_path / "vault" / "_paper_source" / "raw" / "fallback-paper"
...
def test_acquire_paper_from_url_tries_alternate_pdf_urls_from_deduped_sources(tmp_path):
    ...
    paper_root = tmp_path / "vault" / "_paper_source" / "raw" / "alternate-url-paper"
...
def test_acquire_paper_from_url_rejects_html_without_pdf_link(tmp_path):
    server_root = tmp_path / "server"
    server_root.mkdir()
    paper_root = tmp_path / "vault" / "_paper_source" / "raw" / "html-paper"
...
def test_acquire_paper_from_candidate_prefers_paper_search_cli_download(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "paper_source.acquire_papers.read_paper_preview",
        lambda **kwargs: {
            "status": "skipped",
    ...
    paper_root = vault / "_paper_source" / "raw" / "paper-search-cli-paper"
...
def test_acquire_paper_from_candidate_records_paper_search_mcp_download_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("paper_source.acquire_papers.download_paper_pdf", _fake_download_paper_pdf)
    monkeypatch.setattr("paper_source.acquire_papers.read_paper_preview", _fake_read_paper_preview)
    ...
    paper_root = vault / "_paper_source" / "raw" / "paper-search-mcp-paper"
    assert record["status"] == "success"
    ...
    assert seen_preview["save_dir"].name.startswith("paper-source-paper-search-")
...
def test_acquire_paper_from_candidate_soft_fails_retrieval_preview(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("paper_source.acquire_papers.download_paper_pdf", _fake_download_paper_pdf)
    monkeypatch.setattr("paper_source.acquire_papers.read_paper_preview", _failing_read_paper_preview)
    ...
    paper_root = vault / "_paper_source" / "raw" / "preview-soft-fail-paper"
    assert record["status"] == "success"
...
def test_acquire_paper_from_candidate_records_manual_download_when_oa_fallback_has_no_pdf(tmp_path, monkeypatch):
    ...
    assert record["manual_download"]["tmp_manual_pdf_dir"] == "_paper_source/tmp-manual-pdfs"
    assert {
        "kind": "publisher",
        "url": "https://www.sciencedirect.com/science/article/abs/pii/S0029801824027707",
    ...
    assert not (vault / "_paper_source" / "raw" / "manual-download-paper" / "paper.pdf").exists()


def test_acquire_paper_from_url_attaches_manual_download_links_on_access_denied(tmp_path, monkeypatch):
    paper_root = tmp_path / "vault" / "_paper_source" / "raw" / "publisher-blocked-paper"
    ...
    monkeypatch.setattr("paper_source.acquire_papers._download_url_to_temp", _blocked_download)
...
def test_acquire_paper_from_candidate_uses_vault_slug_boundary(tmp_path):
    ...
    paper_root = vault / "_paper_source" / "raw" / "downloaded-paper"
    assert record["status"] == "success"
    assert record["output_path"] == str(paper_root / "paper.pdf")
