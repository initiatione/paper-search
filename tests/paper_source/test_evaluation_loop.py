...
from paper_source.evaluation_loop import BENCHMARK_SCHEMA_VERSION, build_improvement_brief, write_improvement_brief
...
def test_improvement_brief_links_quality_loop_metrics_and_evolution_payload(tmp_path):
    ...
    metric_pack = _write_json(
        tmp_path / "paper-source-quality-gates.json",
        {"metrics": [{"id": "paper-source-quality-gate-pass-rate", "value": 1.0}]},
    )
    ...
    brief = build_improvement_brief(
        ...
        before_metrics={
            ...
            "paper-source-quality-gate-pass-rate": 0.75,
    ...
    assert brief["schema_version"] == "paper-source-improvement-brief-v1"
    ...
    assert brief["source_completeness"] == {
        "required_sources": ["plugin_eval", "paper_source_quality_gates", "benchmark"],
        "present_sources": ["plugin_eval", "paper_source_quality_gates", "benchmark"],
        "missing_sources": [],
        "invalid_sources": [],
        "complete": True,
    }
    ...
    assert comparisons["paper-source-quality-gate-pass-rate"]["trend"] == "improved"
    ...
    assert "paper_source_quality_gate_pass_rate_non_regression" in gate_ids
...
def test_improvement_brief_marks_invalid_benchmark_contract_as_incomplete(tmp_path):
    ...
    assert "schema_version must be paper-source-benchmark-v1" in contract["issues"]
