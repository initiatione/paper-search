...
from paper_source.run_index import auto_prune_run_lifecycle, prune_run_lifecycle
...
def test_run_lifecycle_dry_run_keeps_files_and_reports_candidates(tmp_path):
    ...
    assert (vault / "_paper_source/meta" / "run-lifecycle").is_dir()
...
def test_run_lifecycle_apply_removes_only_terminal_candidates_and_refreshes_index(tmp_path):
    ...
    index = json.loads((vault / "_paper_source/runs" / "index.json").read_text(encoding="utf-8"))
...
def test_auto_run_lifecycle_skips_until_threshold_then_applies(tmp_path):
    ...
    assert len([path for path in (vault / "_paper_source/runs").iterdir() if path.is_dir()]) == 15

    oldest_run = vault / "_paper_source/runs" / "run-00"
...
def test_run_lifecycle_can_prune_failed_invalid_and_stale_running_runs(tmp_path):
    ...
    invalid = vault / "_paper_source/runs" / "run-invalid"
...
def test_repository_cleanup_does_not_remove_review_sessions(tmp_path):
    from paper_source.paper_source_repository import cleanup_paper_source_repository

    vault = tmp_path / "vault"
    review_dir = vault / "_paper_source" / "reviews" / "robotics-abc"
    review_dir.mkdir(parents=True)
    (review_dir / "state.json").write_text('{"schema_version":"paper-source-review-session-v1"}', encoding="utf-8")
