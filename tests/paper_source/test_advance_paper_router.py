...
from paper_source.orchestrator import advance_paper_once
...
def test_advance_paper_once_routes_one_safe_stage_at_a_time(tmp_path):
    ...
    paper_root = vault / "_paper_source" / "raw" / "routed-paper"
    ...
    promotion_plan = vault / "_paper_source" / "staging" / "papers" / "routed-paper" / "promotion-plan.json"
    assert promotion_plan.is_file()
    assert json.loads(promotion_plan.read_text(encoding="utf-8"))["critic_required"] is False
    assert (vault / "_paper_source" / "staging" / "papers" / "routed-paper" / "briefs" / "reading-report.md").is_file()
