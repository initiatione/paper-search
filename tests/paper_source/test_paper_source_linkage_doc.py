...
def test_paper_source_linkage_doc_defines_scope_pipeline_and_sync_rule():
    ...
    assert "_paper_source/evolution/pending" in text
    assert "_paper_source/evolution/rejected" in text
...
def test_paper_source_linkage_doc_matches_current_dry_run_artifact_names():
    text = LINKAGE_DOC.read_text(encoding="utf-8")

    assert "_paper_source/runs/<run-id>/normalized.json" in text
    assert "_paper_source/runs/<run-id>/rank.json" in text
    assert "_paper_source/runs/<run-id>/candidates.normalized.json" not in text
    assert "_paper_source/runs/<run-id>/candidates.ranked.json" not in text


def test_paper_source_linkage_documents_review_sessions_and_evidence_index():
    text = LINKAGE_DOC.read_text(encoding="utf-8")
    assert "_paper_source/reviews/<review-id>/" in text
    assert "candidates.json" in text
    assert "shortlist.json" in text
    assert "fetch_plan.json" in text
    assert "coverage.json" in text
    assert "_paper_source/raw/<slug>/evidence-index.json" in text
    assert "_paper_source/meta/evidence-index.json" in text
