...
阶段 1 命名说明：用户可见名改为 PaperFlow / Paper Source / Paper Wiki；machine-facing 机器名仍暂保留 `paper-search` / `epi` / `prw`，用于安装缓存、历史 artifact、测试和兼容。
...
用户口语别名：PS。PS 只是自然语言别名，不是 `$PS` 入口，也不是独立插件名。Paper Source 机器名仍是 `epi`。
...
用户口语别名：PW。PW 只是自然语言别名，不是 `$PW` 入口，也不是独立插件名。Paper Wiki 机器名仍是 `prw`。
...
仓库根目录承担插件市场源和发布说明职责。Paper Source 插件主体位于 `plugins/epi/`，Paper Wiki 插件主体位于 `plugins/PRW/`。

- `plugins/epi/.codex-plugin/plugin.json`：Paper Source marketplace 元数据、版本号、展示文案和 skill 声明。Paper Source 机器名仍是 `epi`。
- `plugins/PRW/.codex-plugin/plugin.json`：Paper Wiki marketplace 元数据、版本号、展示文案和 skill 声明。Paper Wiki 机器名仍是 `prw`。
- `AGENTS.md`：插件级 thin shell 路由入口，说明如何按任务类型匹配 skill。
- `plugins/epi/skills/`：用户可触发的 Paper Source/EPI skills，包括配置、发现、ingest、MinerU 解析、wiki setup、provenance、run lifecycle、topic tracking 和 Zotero。
- `plugins/PRW/skills/`：Paper Wiki/PRW 的 `paper-research-wiki` 用户入口和 `paper-wiki-language` 支持 gate。
...
添加 marketplace 后，在插件列表中选择 PaperFlow，再安装 `Paper Source` 和 `Paper Wiki`。当前阶段机器名仍是 `epi` / `prw`，所以运行态、安装缓存和历史 artifact 里仍可能显示这些 legacy alias。安装后在新线程中提及 Paper Source、PS 或 `@epi` 可进入论文发现/采集/解析链路；提及 Paper Wiki、PW、`@prw` 或 Paper Research Wiki 可进入正式论文 wiki 写入和维护链路。
