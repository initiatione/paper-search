# Paper Source Stability Refactor Design

## Goal

Make the Paper Source plugin produce stable source-first wiki handoffs after real vault use exposed raw bundle completeness, final-page language, graph visibility, final-source-review, run index fragility, and oversized workflow scripts.
...
- Raw source bundle audit must classify complete and incomplete `_paper_source/raw/<slug>` directories with exact missing artifacts, while keeping legacy `_epi/raw/papers/<slug>` readable for existing artifacts.
...
- Paper Source skills must state the rule that future features should add focused workflow modules rather than growing these router files.
- Paper Source handoff artifacts must carry a clean-worker delegation policy: independent subtasks should go to fresh-context workers, workers should exit after writing bounded outputs, and the main agent should review final artifacts instead of importing long intermediate context.
- Paper Source plugin version must be bumped after implementation and docs/skills must describe the new workflow contract.

## Validation

Focused tests first:

- `tests/paper_source/test_source_bundle_audit.py`
- `tests/paper_source/test_paper_gate.py`
- `tests/paper_source/test_wiki_ingest_record.py`
- `tests/paper_source/test_wiki_init.py`
- `tests/paper_source/test_run_index_dashboard.py`
- `tests/paper_source/test_current_docs.py`

After focused tests pass, run broader Paper Source pytest with repo-local `--basetemp`, validate the plugin manifest, then test the installed or source plugin against `D:\paper-research-wiki` with approval because that vault is outside this workspace sandbox.
