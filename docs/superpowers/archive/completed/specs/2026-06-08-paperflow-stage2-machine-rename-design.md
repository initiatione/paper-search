...
- Python package/module namespace is addressed in Stage 3A; new code should use `paper_source`, with `epi` only as import compatibility
- `_paper_source/` vault artifact root, with `_epi/` as legacy-readable input
- `wiki-ingest-brief.json`, `wiki_deposition_task.json`, `paper-wiki-record-request.json`; legacy `prw-record-request.json` remains readable
- `paper-research-wiki` skill name
- `paper-source-paper-deposition` compatibility skill name
...
- Legacy `plugins/epi` and `plugins/PRW` no longer exist in the Stage 2 branch.
