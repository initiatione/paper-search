...
- Scope: User-visible naming for the existing bundled Paper Source/Paper Wiki plugin family
...
4. Add clear legacy wording: "Paper Source, legacy `epi`" and "Paper Wiki, legacy `prw`" where users might see both names.
...
- Do not rename plugin directories: `plugins/paper-source` and `plugins/paper-wiki` stay in place.
...
Use Paper Source and Paper Wiki only when one of these applies:
...
- The text needs to say that Paper Source was legacy `epi` or Paper Wiki was legacy `prw`.
...
- `plugins/paper-source/.codex-plugin/plugin.json`
  - keep `name`: `epi`
  ...
  - mention `legacy `epi`` where useful
  - keep version prefix in `shortDescription`

- `plugins/paper-wiki/.codex-plugin/plugin.json`
  - keep `name`: `prw`
  ...
  - mention `legacy `prw`` where useful
  - keep version prefix in `shortDescription`
...
- `plugins/paper-source/docs/workflow.md`
- `plugins/paper-source/docs/structure.md`
- `plugins/paper-source/docs/paper-source-linkage.md`
- `plugins/paper-wiki/docs/workflow.md`
- `plugins/paper-wiki/docs/structure.md`
- `plugins/paper-wiki/docs/paper-source-integration.md`
...
- Keep `$paper-research-wiki`, `paper-research-wiki`, `paper-source-paper-deposition`, `wiki-ingest-brief.json`, and other machine/compatibility terms unchanged.

Do not bulk-edit all references files. References that describe artifact contracts may continue using Paper Source/Paper Wiki when those are compatibility or machine terms.

## Boundary Rules

Paper Source does not fail its source workflow because Paper Wiki is absent. Missing Paper Wiki is a capability gap, not an Paper Source/Paper Source source-staging failure.

Paper Wiki diagnoses missing Paper Source-owned vault bootstrap or source artifacts and points back to Paper Source. It does not initialize, reset, repair, or silently create Paper Source-owned structures.
...
- Paper Wiki contract tests should accept new user-facing naming and retain old machine compatibility assertions.
- Paper Source current-doc tests should assert the new names in user-facing docs and preserve legacy compatibility statements.

Verification should include:

```powershell
python -m pytest tests\paper_research_wiki\test_plugin_contract.py plugins\paper-source\tests\test_skill_bundle_contract.py tests\test_marketplace_manifest.py tests\paper_source\test_current_docs.py -q
...
- User-facing docs explain `Paper Source, legacy `epi`` and `Paper Wiki, legacy `prw``.
- `PS` and `PW` appear only as conversational aliases.
- Existing machine-facing names remain unchanged in stage 1: `paper-search`, `epi`, `prw`, `plugins/paper-source`, `plugins/paper-wiki`.
